const getVotos = async () => {
   ...
   // const data = await fetch(`http://localhost/onpe_sweb_php/participacion/${id}`)
    ...
    if ( data.status == 200 ) {
        ...
        let html = ''
    votos.data.forEach(element => {
       html += `<tr onclick="location.href='./participacion_total.html?id=${id}/${element.DPD}'" onmouseover="this.style.cursor = &quot;pointer&quot;; this.style.color = &quot;grey&quot;" onmouseout="this.style.color = &quot;black&quot;" style="cursor: pointer; color: black;">
                          <td>${element.DPD}</td>
                          <td>${element.TV}</td>
                          <td>${element.PTV}</td>
                          <td>${element.TA}</td>
